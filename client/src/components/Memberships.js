...
const Memberships = () => {
    ...
    async function getMemberships() {
        const url = "/members/" + currentUser.email
        try {
    ...
    async function deleteMembership(business) {
        try {
            const response = await fetch(`/deleteMembership/${currentUser.email}/${business}`, {
                method: "DELETE"
            })

            setMemberships(memberships.filter(membership=> membership.business !== business))

            const response2 = await fetch(`/businesses/removeMembersByName/${business}`, {
                method:"PUT",
